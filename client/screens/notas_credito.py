...
class NotaCreditoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('Nueva Nota de Crédito')
    ...
    def _build(self):
        ...
        self.txt_motivo.setPlaceholderText('Motivo de la nota de crédito')
        ...
        left.addRow('Motivo *:', self.txt_motivo)
        ...
        self.txt_desc.setMinimumWidth(200)
        ...
        self.tbl.setHorizontalHeaderLabels(['Descripción', 'Cant.', 'Precio', 'IVA', 'Subtotal'])
        ...
        btns.button(QDialogButtonBox.Ok).setText('Guardar Nota de Crédito')
    ...
    def _refresh_table(self):
        ...
        for d in self._detalles:
            ...
            self.tbl.setItem(r, 0, QTableWidgetItem(d['descripcion']))
            self.tbl.setItem(r, 1, QTableWidgetItem(f'{d["cantidad"]:.2f}'))
            self.tbl.setItem(r, 2, QTableWidgetItem(f'${d["precio_unitario"]:.4f}'))
            self.tbl.setItem(r, 3, QTableWidgetItem(f'{d["impuesto_tarifa"]:.0f}%'))
            self.tbl.setItem(r, 4, QTableWidgetItem(f'${d["precio_total_sin_impuesto"] + d["impuesto_valor"]:.2f}'))
            total += d['precio_total_sin_impuesto'] + d['impuesto_valor']
...
class NotasCreditoScreen(BaseScreen):
    def _setup_ui(self):
        lay = QVBoxLayout(self)
        lay.setContentsMargins(24, 20, 24, 20)
        lay.setSpacing(12)
        title = QLabel('Notas de Crédito')
        ...
        btn_new = make_btn('+ Nueva N. Crédito', 'primary')
        btn_new.clicked.connect(self._new)
        toolbar.addWidget(self.txt_search)
        toolbar.addWidget(btn_new)
        lay.addLayout(toolbar)

        self.table = make_table(['#', 'Fecha', 'Cliente', 'Motivo', 'Total', 'Estado', 'Acciones'])
    ...
    def refresh(self):
        self._run(api.get, self._on_data, f'/api/notas-credito/{api.empresa_id()}')

    def _search(self):
        q = self.txt_search.text().strip()
        self._run(api.get, self._on_data, f'/api/notas-credito/{api.empresa_id()}', params={'q': q})
    ...
    def _on_data(self, result):
        ...
        for row in result['data']:
            ...
            self.table.setItem(r, 3, table_item(row['motivo']))
    ...
    def _new(self):
        dlg = NotaCreditoDialog(self)
        if dlg.exec_() == dlg.Accepted:
            result = api.post('/api/notas-credito/', dlg.get_data())
    ...
    def _autorizar_id(self, nid):
        def do():
            return api.post(f'/api/notas-credito/{nid}/autorizar')
        w = LoadWorker(do)
    ...
    def _pdf(self, data):
        content, err = api.download(f'/api/notas-credito/{data["id"]}/pdf')
        if err:
            show_error(self, err)
            return
        path, _ = QFileDialog.getSaveFileName(self, 'Guardar PDF', f'NC_{data["clave_acceso"]}.pdf', 'PDF (*.pdf)')
        if path:
            with open(path, 'wb') as f:
                f.write(content)

    def _xml(self, data):
        content, err = api.download(f'/api/notas-credito/{data["id"]}/xml')
        if err:
            show_error(self, err)
            return
